...
const Chat = () => {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        setNewMessage('');
        if (!isNameSet) setIsNameSet(true);
        loadMessages();
      }
    } catch (error) {
      toast({
        ...
        <div className="bg-white/95 backdrop-blur-lg rounded-2xl shadow-2xl p-8 max-w-md w-full">
          <div className="text-center mb-6">
            <Avatar className="h-24 w-24 mx-auto mb-4 border-4 border-purple-500/30">
              ...
              <AvatarFallback className="bg-gradient-to-br from-purple-500 to-pink-500 text-2xl">AI</AvatarFallback>
            </Avatar>
            <h2 className="text-2xl font-bold text-gray-800 mb-2">Чат с Al1se</h2>
            <p className="text-gray-600">Представьтесь, чтобы начать общение</p>
          </div>
          
          <form onSubmit={(e) => { e.preventDefault(); if (userName.trim()) setIsNameSet(true); }} className="space-y-4">
            <Input
              ...
              className="text-lg"
            />
            <Button type="submit" className="w-full bg-gradient-to-r from-purple-500 to-pink-600 hover:from-purple-600 hover:to-pink-700 text-white text-lg py-6">
      ...
      <div className="bg-gradient-to-r from-purple-600 to-pink-600 text-white p-4 shadow-lg">
        <div className="max-w-4xl mx-auto flex items-center gap-3">
          ...
          <div className="flex-1">
            <h1 className="font-bold text-lg">Al1se✔️</h1>
      ...
      <div className="flex-1 overflow-y-auto p-4">
        <div className="max-w-4xl mx-auto space-y-4">
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex gap-2 ${isUser ? 'justify-end' : 'justify-start'} animate-fade-in`}>
                {!isUser && (
